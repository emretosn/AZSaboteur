...
```bash
saboteur validate 'AZS_F{c1d9cfd51fe2}'
...
```bash
saboteur validate 'AZS_F{4a9a0d31218b}'
...
```bash
saboteur validate 'AZS_F{923b3bfba141}'
...
|------|--------|-----------|------|
| 1 | NET-MGMT-EXPOSED | Deployment portal cred leak → SSH | `AZS_F{c1d9cfd51fe2}` |
| 2 | CMP-IMDS | SSH with planted creds → IMDS token theft | `AZS_F{4a9a0d31218b}` |
| 3 | STR-KEYVAULT-POLICY | Key Vault secret read with stolen token | `AZS_F{923b3bfba141}` |
