...
class TerraformRunner:
    ...
    def generate_chain_tf(self, chain: list[dict[str, Any]]) -> Path:
        ...
        Cross-step wiring: when a ``keyvault-lax-policy`` step follows a step
        that outputs ``managed_identity_principal_id``, the principal ID is
        passed as ``reader_principal_ids`` so the stolen token can access the
        Key Vault.
        """
        blocks: list[str] = [
        ...
        # Modules whose Terraform output includes managed_identity_principal_id
        _IDENTITY_MODULES = {"modules/web-ssrf", "modules/vm-with-identity"}
        # Modules that accept reader_principal_ids
        _KV_MODULES = {"modules/keyvault-lax-policy"}
        ...
        for step in chain:
            ...
            # Build extra HCL lines for cross-step references
            extra_lines = ""
            if step["module"] in _KV_MODULES:
                # Collect managed_identity_principal_id from all preceding steps
                refs = []
                for prev_idx, prev_name in block_names[:-1]:
                    prev_module = chain[prev_idx]["module"]
                    if prev_module in _IDENTITY_MODULES:
                        refs.append(f"module.{prev_name}.managed_identity_principal_id")
                if refs:
                    refs_str = ", ".join(refs)
                    extra_lines = f"\n                  reader_principal_ids = [{refs_str}]"
            ...
            block = textwrap.dedent(f"""\
                module "{block_name}" {{
                  ...
                  step_index          = {idx}
                  flag                = var.flags["{idx}"]
                  credentials         = var.credentials{extra_lines}
